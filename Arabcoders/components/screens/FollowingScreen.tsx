...
export function FollowingScreen({ userId, onBack, onUserPress }: FollowingScreenProps) {
  ...
  const loadFollowing = async () => {
    try {
      ...
      // استخدام API للـ following
      // جرب endpoints مختلفة
      let response;
      try {
        response = await api.get(`/api/follows/users/${userId}/following`);
      } catch (error: any) {
        // إذا لم يعمل، جرب endpoint آخر
        if (error.response?.status === 404) {
          // قد يكون الـ endpoint مختلف، جرب followers مع query parameter
          response = await api.get(`/api/follows/users/${userId}/followers?type=following`);
        } else {
          throw error;
        }
      }
      ...
      // إذا كان الخطأ 404، يعني لا يوجد following
      if (error.response?.status !== 404) {
        console.error('Error loading following:', error);
      }
      setFollowing([]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>المتابعين</Text>
      ...
      {loading && !refreshing ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>لا يوجد متابعين</Text>
        </View>
      ) : (
        <ScrollView
